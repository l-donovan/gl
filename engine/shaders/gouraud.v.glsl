...
void main(void) {
    vec2 trash = coord_uv;
    mat4 mvp = p * v * m;
    vec3 normalDirection = normalize(m_3x3_inv_transp * coord_norm);
    vec3 viewDirection = normalize(vec3(v_inv * vec4(0.0, 0.0, 0.0, 1.0) - m * coord_4d));
    vec3 lightDirection;
    float attenuation;
    ...
    if (light0.position.w == 0.0) { // directional light
        ...
        vec3 vertexToLightSource = vec3(light0.position - m * coord_4d);

...
void load_obj(const char *filename, vector<glm::vec4> &vertices, vector<glm::vec2> &uvs, vector<glm::vec3> &normals, vector<GLushort> &faces) {
    ...
    normals.resize(rvertices.size(), glm::vec3(0.0));
    vec_count.resize(rvertices.size(), 0);

    bool preset_normals[rvertices.size()];

    for (GLushort vtx_idx = 0; vtx_idx < rvertices.size(); ++vtx_idx) {
        glm::vec3 final_normal(0.0);

        preset_normals[vtx_idx] = false;

        // Average vertex normals
        auto it = vertex_normals.find(vtx_idx);
        if (it != vertex_normals.end()) {
            preset_normals[vtx_idx] = true;

            for (GLushort normal_index : it->second) {
                final_normal += rnormals[normal_index - 1];
            }
        }

        normals[vtx_idx] = glm::normalize(final_normal);
    }
    ...
    for (face_t face : rfaces) {
        ...
        if (!preset_normals[face.v0 - 1] || !preset_normals[face.v1 - 1] || !preset_normals[face.v2 - 1]) {
            ...
            for (GLushort i : { face.v0 - 1, face.v1 - 1, face.v2 - 1 }) {
                ...
                if (vec_count[i] == 1) {
                    ...
                    normals[i].x = normals[i].x * (1.0 - 1.0 / vec_count[i]) + normal.x / vec_count[i];
                    normals[i].y = normals[i].y * (1.0 - 1.0 / vec_count[i]) + normal.y / vec_count[i];
                    normals[i].z = normals[i].z * (1.0 - 1.0 / vec_count[i]) + normal.z / vec_count[i];
    ...
    std::cout << "Loaded mesh " << filename << " (" << vertices.size() << "v, " << (faces.size() / 3) << "f)" << std::endl;
}

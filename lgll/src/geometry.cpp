...
mesh_t load_obj(const char *filename) {
    mesh_t mesh = {
        .vertices = vector<glm::vec4>(),
        .uvs = vector<glm::vec2>(),
        .normals = vector<glm::vec3>(),
        .faces = vector<GLushort>()
    };
        ...
        if (preset_normals) {
            ...
            for (GLushort i : { face.v0 - 1, face.v1 - 1, face.v2 - 1 }) {
                ...
                if (vec_count[i] == 1) {
                    ...
                    mesh.normals[i].x = mesh.normals[i].x * (1.0 - 1.0 / vec_count[i]) + normal.x / vec_count[i];
                    mesh.normals[i].y = mesh.normals[i].y * (1.0 - 1.0 / vec_count[i]) + normal.y / vec_count[i];
                    mesh.normals[i].z = mesh.normals[i].z * (1.0 - 1.0 / vec_count[i]) + normal.z / vec_count[i];
    ...
    std::cout << "Loaded mesh " << filename << " (" << mesh.vertices.size() << "v, " << (mesh.faces.size() / 3) << "f)" << std::endl;
